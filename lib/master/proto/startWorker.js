...
function startWorker(customQueue) {
  ...
  if (this.userDefined && this.userDefined.config && this.userDefined.config.mode === 'docker_debug') {
    process.execArgv.push('--inspect=0.0.0.0');
  }
  ...
  var execArgv = process.execArgv.map(function (option, index) {
    if (option.includes('--inspect=0.0.0.0')) {
      return option;
    }
  ...
  workerProcess.on('exit', function() {
    ...
    for (var pid in q.worker.process) {
      q.worker.list.push(parseInt(pid, 10));
    }
  });
